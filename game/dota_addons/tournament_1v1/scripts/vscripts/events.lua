...
function GameMode:OnGameRulesStateChange(keys)
	...
	if newState == DOTA_GAMERULES_STATE_PRE_GAME then
		if SIDELANES_DISABLED then
			RemoveSideLanes()
		end
	elseif newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
--		if JUNGLE_DISABLED then
--			BlockJungleCamps()
--		end

		if TOWER_TIER_1_INVULNERABLE then
			MakeT1Invulnerable()
		end
	end
end
...
function GameMode:OnNPCSpawned(keys)
	...
	-- make neurals unattackable
	if JUNGLE_DISABLED then
		if npc:GetTeamNumber() == 4 then
			npc:AddNewModifier(npc, nil, "modifier_jungle_blockers", {})
		end
	end

	-- give a free courier
	if FREE_COURIER then
		if npc:IsRealHero() then
			SpawnCourier(npc)
		end
	end

	-- code that should happen on first spawn only
	if not npc.has_spawned then
		npc.has_spawned = true
	end
end
...
function GameMode:OrderFilter( keys )
	...
	if FREE_COURIER then
		if keys.order_type == DOTA_UNIT_ORDER_PURCHASE_ITEM then
			...
			if self.itemIDs[item] == "item_courier" then
	...
	-- restrict the us of shrine if SHRINE_DISABLED option is enabled
	if SHRINE_DISABLED then
